...
- owned `TlsStream` transport with lazy consuming handshakes, native Schannel/OpenSSL providers, system trust and host-name verification, SNI, TLS 1.2 minimum, platform revocation policy, bounded plaintext reads, deadline-aware encrypted I/O, and deterministic close/drop cleanup
...
- nominal `data` schemas plus compiler-owned add/save/find/count/exists/remove syntax lowered through typed HIR/MIR Data plans; scalar count/existence plans avoid source-level list materialization and automatically use eligible indexes or direct native cardinality, while `data memory` executes directly on a DISP-owned native row store and `data open` persists the same plans in fixed-size DISP pages with changed-page WAL recovery, exclusive process locking, backward-readable catalogs, atomic safe schema evolution (optional additions, typed required defaults, and explicit renames), fail-closed destructive drift, field/composite uniqueness and indexes, automatic equality-plan selection, limits, deterministic cleanup, and no SQL surface
