...
| 032 | active | DISP-native typed relational algebra with joins, grouping, aggregates, subqueries, and null semantics | Direct typed `count`/`exists`/`sum`/`average`/`min`/`max` plans execute without source-level list materialization, select field/composite indexes, use O(1) unfiltered native cardinality where applicable, define safe empty-set results, and pass durable interpreter/native differential and compile-fail coverage. Grouping, joins, subqueries, and full null semantics remain |
