...
static bool disp_data_ensure_schema(disp_database_state *state,const char *schema,size_t schema_len,const char *create_sql,size_t create_len,const char *inspect_sql,size_t inspect_len,const char *const *names,const char *const *types,const bool *required,const bool *primary,const bool *unique,const bool *indexed,size_t count,const char *const *constraint_names,const bool *constraint_unique,const size_t *constraint_offsets,const size_t *constraint_fields,size_t constraint_count,disp_native_string *error){
if(state&&state->native)return disp_data_native_schema(state,schema,schema_len,names,types,required,primary,unique,indexed,count,constraint_names,constraint_unique,constraint_offsets,constraint_fields,constraint_count,error);
#ifdef DISP_DATABASE
uint64_t ignored=0;
