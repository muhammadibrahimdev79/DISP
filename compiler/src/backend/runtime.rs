...
typedef struct {disp_native_json key;size_t row;} disp_data_index_entry;typedef struct {disp_native_string name;bool unique;size_t *fields;size_t field_count;disp_data_index_entry *index;size_t index_len;} disp_data_constraint;
typedef struct {char *name;size_t name_len;disp_native_string *field_names;disp_native_string *field_types;bool *required;bool *primary;bool *unique;bool *indexed;disp_data_index_entry **indexes;size_t *index_lens;disp_data_constraint *constraints;size_t constraint_count;size_t field_count;size_t primary_index;disp_native_json *rows;size_t rows_len;size_t rows_cap;} disp_data_table;
...
#endif
static void disp_data_table_drop(disp_data_table *table){disp_dealloc(table->name);for(size_t i=0;i<table->field_count;i++){disp_string_drop(&table->field_names[i]);disp_string_drop(&table->field_types[i]);if(table->indexes&&table->index_lens){for(size_t j=0;j<table->index_lens[i];j++)disp_json_drop(&table->indexes[i][j].key);disp_dealloc(table->indexes[i]);}}disp_dealloc(table->field_names);disp_dealloc(table->field_types);disp_dealloc(table->required);disp_dealloc(table->primary);disp_dealloc(table->unique);disp_dealloc(table->indexed);disp_dealloc(table->indexes);disp_dealloc(table->index_lens);for(size_t i=0;i<table->constraint_count;i++){disp_string_drop(&table->constraints[i].name);disp_dealloc(table->constraints[i].fields);for(size_t j=0;j<table->constraints[i].index_len;j++)disp_json_drop(&table->constraints[i].index[j].key);disp_dealloc(table->constraints[i].index);}disp_dealloc(table->constraints);for(size_t i=0;i<table->rows_len;i++)disp_json_drop(&table->rows[i]);disp_dealloc(table->rows);*table=(disp_data_table){0};}
...
bool ok=disp_data_buffer_u32(payload,(uint32_t)state->tables_len,error);for(size_t t=0;ok&&t<state->tables_len;t++){disp_data_table *table=tables[t];if(t&&disp_data_table_compare(&tables[t-1],&tables[t])==0){ok=disp_data_fail(error,"DISP Data snapshot contains a duplicate table");break;}if(!table->field_count||table->field_count>4096||table->rows_len>DISP_DATABASE_ROW_LIMIT){ok=disp_data_fail(error,"DISP Data table exceeds storage safety limits");break;}ok=disp_data_buffer_bytes(payload,table->name,table->name_len,DISP_DATA_NAME_LIMIT,"table name",error)&&disp_data_buffer_u32(payload,(uint32_t)table->field_count,error);size_t primary_count=0;for(size_t f=0;ok&&f<table->field_count;f++){for(size_t prior=0;prior<f;prior++)if(table->field_names[prior].len==table->field_names[f].len&&!memcmp(table->field_names[prior].data,table->field_names[f].data,table->field_names[f].len)){ok=disp_data_fail(error,"DISP Data table contains a duplicate field");break;}if(!ok)break;ok=disp_data_buffer_bytes(payload,table->field_names[f].data,table->field_names[f].len,DISP_DATA_NAME_LIMIT,"field name",error)&&disp_data_buffer_bytes(payload,table->field_types[f].data,table->field_types[f].len,DISP_DATA_TYPE_LIMIT,"field storage type",error)&&disp_data_buffer_u8(payload,(uint8_t)((table->required[f]?0:1)|(table->primary[f]?2:0)|(table->unique[f]?4:0)|(table->indexed[f]?8:0)),error);if(table->primary[f])primary_count++;}if(ok&&primary_count!=1)ok=disp_data_fail(error,"DISP Data table must contain exactly one primary field");if(ok&&table->constraint_count>4096)ok=disp_data_fail(error,"DISP Data table exceeds 4096 constraints");if(ok)ok=disp_data_buffer_u32(payload,(uint32_t)table->constraint_count,error);for(size_t c=0;ok&&c<table->constraint_count;c++){disp_data_constraint *constraint=&table->constraints[c];for(size_t prior=0;prior<c;prior++)if(table->constraints[prior].name.len==constraint->name.len&&!memcmp(table->constraints[prior].name.data,constraint->name.data,constraint->name.len)){ok=disp_data_fail(error,"DISP Data table contains a duplicate constraint");break;}if(!ok)break;if(constraint->field_count<2||constraint->field_count>table->field_count){ok=disp_data_fail(error,"DISP Data composite constraint has an invalid field count");break;}ok=disp_data_buffer_bytes(payload,constraint->name.data,constraint->name.len,DISP_DATA_NAME_LIMIT,"constraint name",error)&&disp_data_buffer_u8(payload,(uint8_t)(constraint->unique?1:0),error)&&disp_data_buffer_u32(payload,(uint32_t)constraint->field_count,error);for(size_t f=0;ok&&f<constraint->field_count;f++){size_t field=constraint->fields[f];if(field>=table->field_count){ok=disp_data_fail(error,"DISP Data composite constraint references an invalid field");break;}for(size_t prior=0;prior<f;prior++)if(constraint->fields[prior]==field){ok=disp_data_fail(error,"DISP Data composite constraint repeats a field");break;}if(ok)ok=disp_data_buffer_bytes(payload,table->field_names[field].data,table->field_names[field].len,DISP_DATA_NAME_LIMIT,"constraint field",error);}}if(ok)ok=disp_data_buffer_u64(payload,(uint64_t)table->rows_len,error);for(size_t r=0;ok&&r<table->rows_len;r++)ok=disp_data_buffer_bytes(payload,table->rows[r].data,table->rows[r].len,DISP_JSON_LIMIT,"stored row",error);}
...
static bool disp_data_indexes_rebuild(disp_data_table *table,disp_native_string *error);
static bool disp_data_decode(disp_database_state *state,const unsigned char *bytes,size_t length,uint8_t allowed_field_flags,bool supports_constraints,disp_native_string *error){
...
disp_data_reader reader={bytes+DISP_DATA_HEADER_SIZE,bytes+length};uint32_t table_count;if(!disp_data_read_u32(&reader,&table_count,error)||table_count>4096)return error->len?false:disp_data_fail(error,"DISP Data snapshot exceeds 4096 tables");if(table_count){state->tables=(disp_data_table*)disp_alloc_zeroed(table_count,sizeof(disp_data_table),_Alignof(disp_data_table));state->tables_cap=table_count;}for(uint32_t t=0;t<table_count;t++){const char *name;size_t name_len;if(!disp_data_read_bytes(&reader,DISP_DATA_NAME_LIMIT,&name,&name_len,"table name",error)||!disp_utf8_valid(name,name_len))goto fail;for(size_t prior=0;prior<state->tables_len;prior++)if(state->tables[prior].name_len==name_len&&!memcmp(state->tables[prior].name,name,name_len)){disp_data_fail(error,"DISP Data snapshot contains a duplicate table");goto fail;}disp_data_table *table=&state->tables[state->tables_len++];table->name=(char*)disp_alloc(name_len?name_len:1,1);if(name_len)memcpy(table->name,name,name_len);table->name_len=name_len;uint32_t field_count;if(!disp_data_read_u32(&reader,&field_count,error)||!field_count||field_count>4096){if(!error->len)disp_data_fail(error,"DISP Data table must contain between 1 and 4096 fields");goto fail;}table->field_count=field_count;table->primary_index=SIZE_MAX;table->field_names=(disp_native_string*)disp_alloc_zeroed(field_count,sizeof(disp_native_string),_Alignof(disp_native_string));table->field_types=(disp_native_string*)disp_alloc_zeroed(field_count,sizeof(disp_native_string),_Alignof(disp_native_string));table->required=(bool*)disp_alloc(field_count,sizeof(bool));table->primary=(bool*)disp_alloc(field_count,sizeof(bool));table->unique=(bool*)disp_alloc(field_count,sizeof(bool));table->indexed=(bool*)disp_alloc(field_count,sizeof(bool));size_t primary_count=0;for(uint32_t f=0;f<field_count;f++){const char *field_name,*field_type;size_t field_name_len,field_type_len;uint8_t field_flags;if(!disp_data_read_bytes(&reader,DISP_DATA_NAME_LIMIT,&field_name,&field_name_len,"field name",error)||!disp_utf8_valid(field_name,field_name_len)||!disp_data_read_bytes(&reader,DISP_DATA_TYPE_LIMIT,&field_type,&field_type_len,"field storage type",error)||!disp_utf8_valid(field_type,field_type_len)||!disp_data_read_u8(&reader,&field_flags,error))goto fail;if(field_flags&~allowed_field_flags){disp_data_fail(error,"DISP Data field uses unknown required flags");goto fail;}for(uint32_t prior=0;prior<f;prior++)if(table->field_names[prior].len==field_name_len&&!memcmp(table->field_names[prior].data,field_name,field_name_len)){disp_data_fail(error,"DISP Data table contains a duplicate field");goto fail;}table->field_names[f]=disp_owned_bytes(field_name,field_name_len);table->field_types[f]=disp_owned_bytes(field_type,field_type_len);table->required[f]=(field_flags&1u)==0;table->primary[f]=(field_flags&2u)!=0;table->unique[f]=(field_flags&4u)!=0;table->indexed[f]=(field_flags&8u)!=0;if(table->primary[f]){primary_count++;table->primary_index=f;}}if(primary_count!=1){disp_data_fail(error,"DISP Data table must contain exactly one primary field");goto fail;}uint32_t constraint_count=0;if(supports_constraints&&!disp_data_read_u32(&reader,&constraint_count,error))goto fail;if(constraint_count>4096){disp_data_fail(error,"DISP Data table exceeds 4096 constraints");goto fail;}if(constraint_count)table->constraints=(disp_data_constraint*)disp_alloc_zeroed(constraint_count,sizeof(disp_data_constraint),_Alignof(disp_data_constraint));for(uint32_t c=0;c<constraint_count;c++){const char *constraint_name;size_t constraint_name_len;uint8_t constraint_flags;uint32_t constrained_count;if(!disp_data_read_bytes(&reader,DISP_DATA_NAME_LIMIT,&constraint_name,&constraint_name_len,"constraint name",error)||!disp_utf8_valid(constraint_name,constraint_name_len)||!disp_data_read_u8(&reader,&constraint_flags,error)||!disp_data_read_u32(&reader,&constrained_count,error))goto fail;if(constraint_flags&~1u||constrained_count<2||constrained_count>field_count){disp_data_fail(error,"DISP Data composite constraint metadata is invalid");goto fail;}for(uint32_t field=0;field<field_count;field++)if(table->field_names[field].len==constraint_name_len&&!memcmp(table->field_names[field].data,constraint_name,constraint_name_len)){disp_data_fail(error,"DISP Data field and constraint names must be distinct");goto fail;}for(uint32_t prior=0;prior<c;prior++)if(table->constraints[prior].name.len==constraint_name_len&&!memcmp(table->constraints[prior].name.data,constraint_name,constraint_name_len)){disp_data_fail(error,"DISP Data table contains a duplicate constraint");goto fail;}disp_data_constraint *constraint=&table->constraints[table->constraint_count++];constraint->name=disp_owned_bytes(constraint_name,constraint_name_len);constraint->unique=(constraint_flags&1u)!=0;constraint->field_count=constrained_count;constraint->fields=(size_t*)disp_alloc(constrained_count,sizeof(size_t));for(uint32_t item=0;item<constrained_count;item++){const char *constrained_name;size_t constrained_name_len;if(!disp_data_read_bytes(&reader,DISP_DATA_NAME_LIMIT,&constrained_name,&constrained_name_len,"constraint field",error)||!disp_utf8_valid(constrained_name,constrained_name_len))goto fail;size_t found=SIZE_MAX;for(uint32_t field=0;field<field_count;field++)if(table->field_names[field].len==constrained_name_len&&!memcmp(table->field_names[field].data,constrained_name,constrained_name_len)){found=field;break;}if(found==SIZE_MAX){disp_data_fail(error,"DISP Data composite constraint references an unknown field");goto fail;}for(uint32_t prior=0;prior<item;prior++)if(constraint->fields[prior]==found){disp_data_fail(error,"DISP Data composite constraint repeats a field");goto fail;}constraint->fields[item]=found;}}uint64_t row_count;if(!disp_data_read_u64(&reader,&row_count,error)||row_count>DISP_DATABASE_ROW_LIMIT||row_count>SIZE_MAX/sizeof(disp_native_json)){if(!error->len)disp_data_fail(error,"DISP Data table exceeds the 100000-row limit");goto fail;}if(row_count){table->rows=(disp_native_json*)disp_alloc_zeroed((size_t)row_count,sizeof(disp_native_json),_Alignof(disp_native_json));table->rows_cap=(size_t)row_count;}for(uint64_t r=0;r<row_count;r++){const char *row;size_t row_len;if(!disp_data_read_bytes(&reader,DISP_JSON_LIMIT,&row,&row_len,"stored row",error)||!disp_json_parse(row,row_len,&table->rows[table->rows_len],error))goto fail;table->rows_len++;}if(!disp_data_indexes_rebuild(table,error))goto fail;}
...
static int disp_data_index_compare(const disp_native_json *left,const disp_native_json *right){size_t count=left->len<right->len?left->len:right->len;int order=count?memcmp(left->data,right->data,count):0;if(order)return order;return left->len<right->len?-1:left->len>right->len?1:0;}
static void disp_data_indexes_clear(disp_data_table *table){if(table->indexes&&table->index_lens)for(size_t field=0;field<table->field_count;field++){for(size_t index=0;index<table->index_lens[field];index++)disp_json_drop(&table->indexes[field][index].key);disp_dealloc(table->indexes[field]);table->indexes[field]=NULL;table->index_lens[field]=0;}for(size_t constraint=0;constraint<table->constraint_count;constraint++){for(size_t index=0;index<table->constraints[constraint].index_len;index++)disp_json_drop(&table->constraints[constraint].index[index].key);disp_dealloc(table->constraints[constraint].index);table->constraints[constraint].index=NULL;table->constraints[constraint].index_len=0;}}
static bool disp_data_values_key(const disp_native_json *values,size_t count,disp_native_json *key,disp_native_string *error){if(count<2||count>4096)return disp_data_fail(error,"DISP Data composite key has an invalid field count");size_t length=2+(count-1);for(size_t i=0;i<count;i++){if(values[i].len>DISP_JSON_LIMIT||length>DISP_JSON_LIMIT-values[i].len)return disp_data_fail(error,"DISP Data composite key exceeds its storage limit");length+=values[i].len;}key->data=(char*)disp_alloc(length?length:1,1);size_t at=0;key->data[at++]='[';for(size_t i=0;i<count;i++){if(i)key->data[at++]=',';if(values[i].len){memcpy(key->data+at,values[i].data,values[i].len);at+=values[i].len;}}key->data[at++]=']';key->len=key->cap=at;return true;}
static bool disp_data_constraint_key(disp_data_table *table,const disp_data_constraint *constraint,const disp_native_json *row,disp_native_json *key,disp_native_string *error){disp_native_json *values=(disp_native_json*)disp_alloc_zeroed(constraint->field_count,sizeof(disp_native_json),_Alignof(disp_native_json));size_t done=0;for(;done<constraint->field_count;done++){size_t field=constraint->fields[done];disp_native_string *name=&table->field_names[field];if(!disp_json_get(row,name->data,name->len,&values[done])){for(size_t i=0;i<done;i++)disp_json_drop(&values[i]);disp_dealloc(values);return disp_data_fail(error,"stored row is missing a composite-index field");}}bool ok=disp_data_values_key(values,constraint->field_count,key,error);for(size_t i=0;i<constraint->field_count;i++)disp_json_drop(&values[i]);disp_dealloc(values);return ok;}
static bool disp_data_indexes_rebuild(disp_data_table *table,disp_native_string *error){disp_data_indexes_clear(table);if(!table->indexes)table->indexes=(disp_data_index_entry**)disp_alloc_zeroed(table->field_count,sizeof(disp_data_index_entry*),_Alignof(disp_data_index_entry*));if(!table->index_lens)table->index_lens=(size_t*)disp_alloc_zeroed(table->field_count,sizeof(size_t),_Alignof(size_t));for(size_t field=0;field<table->field_count;field++){if(!table->primary[field]&&!table->unique[field]&&!table->indexed[field])continue;if(table->rows_len)table->indexes[field]=(disp_data_index_entry*)disp_alloc_zeroed(table->rows_len,sizeof(disp_data_index_entry),_Alignof(disp_data_index_entry));for(size_t row=0;row<table->rows_len;row++){disp_native_json key={0};disp_native_string *name=&table->field_names[field];if(!disp_json_get(&table->rows[row],name->data,name->len,&key)){disp_data_indexes_clear(table);return disp_data_fail(error,"stored row is missing an indexed field");}if(table->primary[field]||table->unique[field]){for(size_t existing=0;existing<table->index_lens[field];existing++)if(disp_data_index_compare(&table->indexes[field][existing].key,&key)==0){disp_json_drop(&key);disp_data_indexes_clear(table);return disp_data_fail(error,"stored table contains a duplicate indexed value");}}size_t at=table->index_lens[field];while(at&&disp_data_index_compare(&table->indexes[field][at-1].key,&key)>0){table->indexes[field][at]=table->indexes[field][at-1];at--;}table->indexes[field][at]=(disp_data_index_entry){.key=key,.row=row};table->index_lens[field]++;}}for(size_t constraint=0;constraint<table->constraint_count;constraint++){disp_data_constraint *item=&table->constraints[constraint];if(table->rows_len)item->index=(disp_data_index_entry*)disp_alloc_zeroed(table->rows_len,sizeof(disp_data_index_entry),_Alignof(disp_data_index_entry));for(size_t row=0;row<table->rows_len;row++){disp_native_json key={0};if(!disp_data_constraint_key(table,item,&table->rows[row],&key,error)){disp_data_indexes_clear(table);return false;}if(item->unique){for(size_t existing=0;existing<item->index_len;existing++)if(disp_data_index_compare(&item->index[existing].key,&key)==0){disp_json_drop(&key);disp_data_indexes_clear(table);return disp_data_fail(error,"stored table violates a composite unique constraint");}}size_t at=item->index_len;while(at&&disp_data_index_compare(&item->index[at-1].key,&key)>0){item->index[at]=item->index[at-1];at--;}item->index[at]=(disp_data_index_entry){.key=key,.row=row};item->index_len++;}}return true;}
static bool disp_data_index_find(disp_data_table *table,size_t field,const disp_native_json *key,size_t *row){if(field>=table->field_count||!table->indexes||!table->index_lens)return false;size_t low=0,high=table->index_lens[field];while(low<high){size_t middle=low+(high-low)/2;int order=disp_data_index_compare(&table->indexes[field][middle].key,key);if(order<0)low=middle+1;else high=middle;}if(low<table->index_lens[field]&&!disp_data_index_compare(&table->indexes[field][low].key,key)){*row=table->indexes[field][low].row;return true;}return false;}
static bool disp_data_migrate_row(const disp_data_table *table,const disp_native_json *row,const char *const *names,const char *const *defaults,const size_t *default_lens,size_t count,disp_native_json *migrated,disp_native_string *error){
disp_native_string *keys=(disp_native_string*)disp_alloc_zeroed(count,sizeof(disp_native_string),_Alignof(disp_native_string));
disp_native_json *values=(disp_native_json*)disp_alloc_zeroed(count,sizeof(disp_native_json),_Alignof(disp_native_json));
size_t done=0;
for(;done<count;done++){
keys[done]=(disp_native_string){.data=(char*)names[done],.len=strlen(names[done]),.cap=0};
if(done<table->field_count){
disp_native_string *stored=&table->field_names[done];
if(!disp_json_get(row,stored->data,stored->len,&values[done])){
disp_data_fail(error,"stored row is missing a DISP Data field");
goto failed;
}
}else if(defaults[done]){
values[done]=disp_json_literal(defaults[done],default_lens[done]);
}else{
values[done]=disp_json_literal("null",4);
}
}
{
bool ok=disp_json_from_object(keys,values,count,migrated,error);
for(size_t i=0;i<count;i++)disp_json_drop(&values[i]);
disp_dealloc(values);
disp_dealloc(keys);
return ok;
}
failed:
for(size_t i=0;i<done;i++)disp_json_drop(&values[i]);
disp_dealloc(values);
disp_dealloc(keys);
return false;
}
static bool disp_data_native_evolve(disp_database_state *state,disp_data_table *table,const char *const *names,const char *const *types,const bool *required,const bool *primary,const bool *unique,const bool *indexed,const char *const *migration_from,const char *const *defaults,const size_t *default_lens,size_t count,const char *const *constraint_names,const bool *constraint_unique,const size_t *constraint_offsets,const size_t *constraint_fields,size_t constraint_count,disp_native_string *error){if(table->field_count>count)return disp_data_fail(error,"the current DISP Data schema removes stored fields");bool changed=table->field_count!=count;for(size_t i=0;i<table->field_count;i++){bool same_name=table->field_names[i].len==strlen(names[i])&&!memcmp(table->field_names[i].data,names[i],table->field_names[i].len);if(!same_name){if(!migration_from[i]||table->field_names[i].len!=strlen(migration_from[i])||memcmp(table->field_names[i].data,migration_from[i],table->field_names[i].len))return disp_data_fail(error,"a renamed DISP Data field requires the matching from(old_name) migration");changed=true;}if(table->field_types[i].len!=strlen(types[i])||memcmp(table->field_types[i].data,types[i],table->field_types[i].len)||table->required[i]!=required[i]||table->primary[i]!=primary[i])return disp_data_fail(error,"the current DISP Data schema changes a stored field type, optionality, or primary identity");if((table->unique[i]&&!unique[i])||(table->indexed[i]&&!indexed[i]))return disp_data_fail(error,"the current DISP Data schema removes a stored constraint or index");if(table->unique[i]!=unique[i]||table->indexed[i]!=indexed[i])changed=true;}for(size_t i=table->field_count;i<count;i++)if((required[i]&&!defaults[i])||primary[i])return disp_data_fail(error,"a new DISP Data field must be optional or declare default(value)");if(table->constraint_count>constraint_count)return disp_data_fail(error,"the current DISP Data schema removes a named constraint");for(size_t c=0;c<table->constraint_count;c++){disp_data_constraint *stored=&table->constraints[c];size_t begin=constraint_offsets[c],end=constraint_offsets[c+1];if(end<begin||stored->name.len!=strlen(constraint_names[c])||memcmp(stored->name.data,constraint_names[c],stored->name.len)||stored->unique!=constraint_unique[c]||stored->field_count!=end-begin)return disp_data_fail(error,"the current DISP Data schema reorders or changes a named constraint");for(size_t f=0;f<stored->field_count;f++)if(stored->fields[f]!=constraint_fields[begin+f])return disp_data_fail(error,"the current DISP Data schema changes constrained fields");}if(table->constraint_count!=constraint_count)changed=true;if(!changed)return true;disp_data_table migrated={0};migrated.name=(char*)disp_alloc(table->name_len?table->name_len:1,1);if(table->name_len)memcpy(migrated.name,table->name,table->name_len);migrated.name_len=table->name_len;migrated.field_count=count;migrated.primary_index=SIZE_MAX;migrated.field_names=(disp_native_string*)disp_alloc_zeroed(count,sizeof(disp_native_string),_Alignof(disp_native_string));migrated.field_types=(disp_native_string*)disp_alloc_zeroed(count,sizeof(disp_native_string),_Alignof(disp_native_string));migrated.required=(bool*)disp_alloc(count,sizeof(bool));migrated.primary=(bool*)disp_alloc(count,sizeof(bool));migrated.unique=(bool*)disp_alloc(count,sizeof(bool));migrated.indexed=(bool*)disp_alloc(count,sizeof(bool));for(size_t i=0;i<count;i++){migrated.field_names[i]=disp_owned_bytes(names[i],strlen(names[i]));migrated.field_types[i]=disp_owned_bytes(types[i],strlen(types[i]));migrated.required[i]=required[i];migrated.primary[i]=primary[i];migrated.unique[i]=unique[i];migrated.indexed[i]=indexed[i];if(primary[i]){if(migrated.primary_index!=SIZE_MAX)goto invalid;migrated.primary_index=i;}}if(migrated.primary_index==SIZE_MAX)goto invalid;if(constraint_count)migrated.constraints=(disp_data_constraint*)disp_alloc_zeroed(constraint_count,sizeof(disp_data_constraint),_Alignof(disp_data_constraint));for(size_t c=0;c<constraint_count;c++){size_t begin=constraint_offsets[c],end=constraint_offsets[c+1];if(end<begin||end-begin<2||end-begin>count)goto invalid;disp_data_constraint *constraint=&migrated.constraints[migrated.constraint_count++];constraint->name=disp_owned_bytes(constraint_names[c],strlen(constraint_names[c]));constraint->unique=constraint_unique[c];constraint->field_count=end-begin;constraint->fields=(size_t*)disp_alloc(constraint->field_count,sizeof(size_t));for(size_t f=0;f<constraint->field_count;f++){size_t field=constraint_fields[begin+f];if(field>=count)goto invalid;for(size_t prior=0;prior<f;prior++)if(constraint->fields[prior]==field)goto invalid;constraint->fields[f]=field;}}migrated.rows_len=migrated.rows_cap=table->rows_len;if(table->rows_len)migrated.rows=(disp_native_json*)disp_alloc_zeroed(table->rows_len,sizeof(disp_native_json),_Alignof(disp_native_json));for(size_t row=0;row<table->rows_len;row++)if(!disp_data_migrate_row(table,&table->rows[row],names,defaults,default_lens,count,&migrated.rows[row],error)){disp_data_table_drop(&migrated);return false;}if(!disp_data_indexes_rebuild(&migrated,error)){disp_data_table_drop(&migrated);return false;}{disp_data_table previous=*table;*table=migrated;if(!disp_data_commit(state,error)){migrated=*table;*table=previous;disp_data_table_drop(&migrated);return false;}disp_data_table_drop(&previous);return true;}invalid:disp_data_table_drop(&migrated);return disp_data_fail(error,"the evolved DISP Data schema is invalid");}
static bool disp_data_native_schema(disp_database_state *state,const char *schema,size_t schema_len,const char *const *names,const char *const *types,const bool *required,const bool *primary,const bool *unique,const bool *indexed,const char *const *migration_from,const char *const *defaults,const size_t *default_lens,size_t count,const char *const *constraint_names,const bool *constraint_unique,const size_t *constraint_offsets,const size_t *constraint_fields,size_t constraint_count,disp_native_string *error){disp_data_table *table=disp_data_native_table(state,schema,schema_len);if(table)return disp_data_native_evolve(state,table,names,types,required,primary,unique,indexed,migration_from,defaults,default_lens,count,constraint_names,constraint_unique,constraint_offsets,constraint_fields,constraint_count,error);if(!state||!state->native||state->closed){const char *message="data store is closed";*error=disp_owned_bytes(message,strlen(message));return false;}if(!count||count>4096||constraint_count>4096||state->tables_len>=4096){const char *message="DISP Data schema exceeds storage safety limits";*error=disp_owned_bytes(message,strlen(message));return false;}if(state->tables_len==state->tables_cap){size_t capacity=state->tables_cap?state->tables_cap*2:4;state->tables=(disp_data_table*)disp_realloc(state->tables,capacity*sizeof(disp_data_table),_Alignof(disp_data_table));memset(state->tables+state->tables_cap,0,(capacity-state->tables_cap)*sizeof(disp_data_table));state->tables_cap=capacity;}table=&state->tables[state->tables_len];table->name=(char*)disp_alloc(schema_len?schema_len:1,1);if(schema_len)memcpy(table->name,schema,schema_len);table->name_len=schema_len;table->field_names=(disp_native_string*)disp_alloc_zeroed(count,sizeof(disp_native_string),_Alignof(disp_native_string));table->field_types=(disp_native_string*)disp_alloc_zeroed(count,sizeof(disp_native_string),_Alignof(disp_native_string));table->required=(bool*)disp_alloc(count,sizeof(bool));table->primary=(bool*)disp_alloc(count,sizeof(bool));table->unique=(bool*)disp_alloc(count,sizeof(bool));table->indexed=(bool*)disp_alloc(count,sizeof(bool));table->field_count=count;table->primary_index=SIZE_MAX;for(size_t i=0;i<count;i++){table->field_names[i]=disp_owned_bytes(names[i],strlen(names[i]));table->field_types[i]=disp_owned_bytes(types[i],strlen(types[i]));table->required[i]=required[i];table->primary[i]=primary[i];table->unique[i]=unique[i];table->indexed[i]=indexed[i];if(primary[i]){if(table->primary_index!=SIZE_MAX)goto invalid;table->primary_index=i;}}if(table->primary_index==SIZE_MAX)goto invalid;if(constraint_count)table->constraints=(disp_data_constraint*)disp_alloc_zeroed(constraint_count,sizeof(disp_data_constraint),_Alignof(disp_data_constraint));for(size_t c=0;c<constraint_count;c++){size_t begin=constraint_offsets[c],end=constraint_offsets[c+1];if(end<begin||end-begin<2||end-begin>count)goto invalid;size_t name_len=strlen(constraint_names[c]);if(!name_len||name_len>DISP_DATA_NAME_LIMIT)goto invalid;for(size_t f=0;f<count;f++)if(strlen(names[f])==name_len&&!memcmp(names[f],constraint_names[c],name_len))goto invalid;for(size_t prior=0;prior<c;prior++)if(table->constraints[prior].name.len==name_len&&!memcmp(table->constraints[prior].name.data,constraint_names[c],name_len))goto invalid;disp_data_constraint *constraint=&table->constraints[table->constraint_count++];constraint->name=disp_owned_bytes(constraint_names[c],name_len);constraint->unique=constraint_unique[c];constraint->field_count=end-begin;constraint->fields=(size_t*)disp_alloc(constraint->field_count,sizeof(size_t));for(size_t f=0;f<constraint->field_count;f++){size_t field=constraint_fields[begin+f];if(field>=count)goto invalid;for(size_t prior=0;prior<f;prior++)if(constraint->fields[prior]==field)goto invalid;constraint->fields[f]=field;}}state->tables_len++;if(disp_data_commit(state,error))return true;state->tables_len--;disp_data_table_drop(table);return false;invalid:disp_data_table_drop(table);{const char *message="DISP Data schema must have exactly one primary field";*error=disp_owned_bytes(message,strlen(message));return false;}}
static bool disp_data_native_unique(disp_data_table *table,const disp_native_json *row,size_t skip,disp_native_string *error){for(size_t field=0;field<table->field_count;field++){if(!table->unique[field])continue;disp_native_string *name=&table->field_names[field];disp_native_json candidate={0};if(!disp_json_get(row,name->data,name->len,&candidate)){*error=disp_owned_bytes("data value is missing a unique field",strlen("data value is missing a unique field"));return false;}size_t existing=SIZE_MAX;bool duplicate=disp_data_index_find(table,field,&candidate,&existing)&&existing!=skip;disp_json_drop(&candidate);if(duplicate){*error=disp_owned_bytes("duplicate unique value in DISP Data table",strlen("duplicate unique value in DISP Data table"));return false;}}return true;}
static bool disp_data_native_write(disp_database_state *state,const char *schema,size_t schema_len,const disp_native_json *row,bool replace,uint64_t *changes,disp_native_string *error){*changes=0;disp_data_table *table=disp_data_native_table(state,schema,schema_len);if(!table){const char *message="DISP Data schema is not registered";*error=disp_owned_bytes(message,strlen(message));return false;}disp_native_json key={0};disp_native_string *primary=&table->field_names[table->primary_index];if(!disp_json_get(row,primary->data,primary->len,&key)){const char *message="data value is missing its primary field";*error=disp_owned_bytes(message,strlen(message));return false;}size_t existing=SIZE_MAX;bool found=disp_data_index_find(table,table->primary_index,&key,&existing);disp_json_drop(&key);if(found){if(!replace){const char *message="duplicate primary value in DISP Data table";*error=disp_owned_bytes(message,strlen(message));return false;}if(!disp_data_native_unique(table,row,existing,error))return false;disp_native_json previous=table->rows[existing],replacement=disp_json_literal(row->data,row->len);table->rows[existing]=replacement;if(!disp_data_indexes_rebuild(table,error)){table->rows[existing]=previous;disp_json_drop(&replacement);disp_native_string ignored={0};disp_data_indexes_rebuild(table,&ignored);disp_string_drop(&ignored);return false;}if(!disp_data_commit(state,error)){table->rows[existing]=previous;disp_json_drop(&replacement);disp_native_string ignored={0};disp_data_indexes_rebuild(table,&ignored);disp_string_drop(&ignored);return false;}disp_json_drop(&previous);*changes=1;return true;}if(!disp_data_native_unique(table,row,SIZE_MAX,error))return false;if(table->rows_len>=DISP_DATABASE_ROW_LIMIT){const char *message="data table exceeds the 100000-row limit";*error=disp_owned_bytes(message,strlen(message));return false;}if(table->rows_len==table->rows_cap){size_t capacity=table->rows_cap?table->rows_cap*2:8;if(capacity>DISP_DATABASE_ROW_LIMIT)capacity=DISP_DATABASE_ROW_LIMIT;table->rows=(disp_native_json*)disp_realloc(table->rows,capacity*sizeof(disp_native_json),_Alignof(disp_native_json));table->rows_cap=capacity;}table->rows[table->rows_len++]=disp_json_literal(row->data,row->len);if(!disp_data_indexes_rebuild(table,error)){disp_json_drop(&table->rows[--table->rows_len]);disp_native_string ignored={0};disp_data_indexes_rebuild(table,&ignored);disp_string_drop(&ignored);return false;}if(!disp_data_commit(state,error)){disp_json_drop(&table->rows[--table->rows_len]);disp_native_string ignored={0};disp_data_indexes_rebuild(table,&ignored);disp_string_drop(&ignored);return false;}*changes=1;return true;}
static bool disp_data_native_length(disp_database_state *state,const char *schema,size_t schema_len,uint64_t *length,disp_native_string *error){disp_data_table *table=disp_data_native_table(state,schema,schema_len);if(!table)return disp_data_fail(error,"DISP Data schema is not registered");*length=(uint64_t)table->rows_len;return true;}
...
static bool disp_data_native_lookup(disp_database_state *state,const char *schema,size_t schema_len,size_t field,const disp_native_json *key,disp_native_json **rows,size_t *rows_len,size_t *rows_cap,disp_native_string *error){*rows=NULL;*rows_len=*rows_cap=0;disp_data_table *table=disp_data_native_table(state,schema,schema_len);if(!table){const char *message="DISP Data schema is not registered";*error=disp_owned_bytes(message,strlen(message));return false;}if(field>=table->field_count||(!table->primary[field]&&!table->unique[field]&&!table->indexed[field])){const char *message="DISP Data plan selected an invalid index";*error=disp_owned_bytes(message,strlen(message));return false;}size_t first=0,end=table->index_lens[field];while(first<end){size_t middle=first+(end-first)/2;if(disp_data_index_compare(&table->indexes[field][middle].key,key)<0)first=middle+1;else end=middle;}if(first>=table->index_lens[field]||disp_data_index_compare(&table->indexes[field][first].key,key)!=0)return true;size_t last=first+1;while(last<table->index_lens[field]&&disp_data_index_compare(&table->indexes[field][last].key,key)==0)last++;size_t count=last-first;*rows=(disp_native_json*)disp_alloc_zeroed(count,sizeof(disp_native_json),_Alignof(disp_native_json));for(size_t i=0;i<count;i++){size_t row=table->indexes[field][first+i].row;if(row>=table->rows_len){for(size_t prior=0;prior<i;prior++)disp_json_drop(&(*rows)[prior]);disp_dealloc(*rows);*rows=NULL;const char *message="DISP Data index contains an invalid row";*error=disp_owned_bytes(message,strlen(message));return false;}(*rows)[i]=disp_json_literal(table->rows[row].data,table->rows[row].len);}*rows_len=*rows_cap=count;return true;}
static bool disp_data_native_composite_lookup(disp_database_state *state,const char *schema,size_t schema_len,size_t constraint_index,const disp_native_json *values,size_t value_count,disp_native_json **rows,size_t *rows_len,size_t *rows_cap,disp_native_string *error){*rows=NULL;*rows_len=*rows_cap=0;disp_data_table *table=disp_data_native_table(state,schema,schema_len);if(!table)return disp_data_fail(error,"DISP Data schema is not registered");if(constraint_index>=table->constraint_count)return disp_data_fail(error,"DISP Data plan selected an invalid composite index");disp_data_constraint *constraint=&table->constraints[constraint_index];if(value_count!=constraint->field_count)return disp_data_fail(error,"DISP Data composite lookup has the wrong key width");disp_native_json key={0};if(!disp_data_values_key(values,value_count,&key,error))return false;size_t first=0,end=constraint->index_len;while(first<end){size_t middle=first+(end-first)/2;if(disp_data_index_compare(&constraint->index[middle].key,&key)<0)first=middle+1;else end=middle;}if(first>=constraint->index_len||disp_data_index_compare(&constraint->index[first].key,&key)!=0){disp_json_drop(&key);return true;}size_t last=first+1;while(last<constraint->index_len&&disp_data_index_compare(&constraint->index[last].key,&key)==0)last++;disp_json_drop(&key);size_t count=last-first;*rows=(disp_native_json*)disp_alloc_zeroed(count,sizeof(disp_native_json),_Alignof(disp_native_json));for(size_t i=0;i<count;i++){size_t row=constraint->index[first+i].row;if(row>=table->rows_len){for(size_t prior=0;prior<i;prior++)disp_json_drop(&(*rows)[prior]);disp_dealloc(*rows);*rows=NULL;return disp_data_fail(error,"DISP Data composite index contains an invalid row");}(*rows)[i]=disp_json_literal(table->rows[row].data,table->rows[row].len);}*rows_len=*rows_cap=count;return true;}
static bool disp_data_native_delete(disp_database_state *state,const char *schema,size_t schema_len,const bool *remove,size_t count,uint64_t *changes,disp_native_string *error){*changes=0;disp_data_table *table=disp_data_native_table(state,schema,schema_len);if(!table){const char *message="DISP Data schema is not registered";*error=disp_owned_bytes(message,strlen(message));return false;}if(count!=table->rows_len){const char *message="DISP Data table changed during removal";*error=disp_owned_bytes(message,strlen(message));return false;}size_t kept=0;for(size_t i=0;i<count;i++){if(remove[i])(*changes)++;else kept++;}if(!*changes)return true;disp_native_json *replacement=kept?(disp_native_json*)disp_alloc_zeroed(kept,sizeof(disp_native_json),_Alignof(disp_native_json)):NULL;size_t at=0;for(size_t i=0;i<count;i++)if(!remove[i])replacement[at++]=disp_json_literal(table->rows[i].data,table->rows[i].len);disp_native_json *previous=table->rows;size_t previous_len=table->rows_len,previous_cap=table->rows_cap;table->rows=replacement;table->rows_len=table->rows_cap=kept;if(!disp_data_indexes_rebuild(table,error)){for(size_t i=0;i<kept;i++)disp_json_drop(&replacement[i]);disp_dealloc(replacement);table->rows=previous;table->rows_len=previous_len;table->rows_cap=previous_cap;disp_native_string ignored={0};disp_data_indexes_rebuild(table,&ignored);disp_string_drop(&ignored);*changes=0;return false;}if(!disp_data_commit(state,error)){for(size_t i=0;i<kept;i++)disp_json_drop(&replacement[i]);disp_dealloc(replacement);table->rows=previous;table->rows_len=previous_len;table->rows_cap=previous_cap;disp_native_string ignored={0};disp_data_indexes_rebuild(table,&ignored);disp_string_drop(&ignored);*changes=0;return false;}for(size_t i=0;i<previous_len;i++)disp_json_drop(&previous[i]);disp_dealloc(previous);return true;}
static bool disp_data_ensure_schema(disp_database_state *state,const char *schema,size_t schema_len,const char *create_sql,size_t create_len,const char *inspect_sql,size_t inspect_len,const char *const *names,const char *const *types,const bool *required,const bool *primary,const bool *unique,const bool *indexed,const char *const *migration_from,const char *const *defaults,const size_t *default_lens,size_t count,const char *const *constraint_names,const bool *constraint_unique,const size_t *constraint_offsets,const size_t *constraint_fields,size_t constraint_count,disp_native_string *error){
if(state&&state->native)return disp_data_native_schema(state,schema,schema_len,names,types,required,primary,unique,indexed,migration_from,defaults,default_lens,count,constraint_names,constraint_unique,constraint_offsets,constraint_fields,constraint_count,error);
#ifdef DISP_DATABASE
uint64_t ignored=0;
