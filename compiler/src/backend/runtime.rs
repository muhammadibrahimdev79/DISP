...
typedef struct {char *name;size_t name_len;disp_native_string *field_names;disp_native_string *field_types;bool *required;bool *primary;size_t field_count;size_t primary_index;disp_native_json *rows;size_t rows_len;size_t rows_cap;} disp_data_table;
...
#endif
static void disp_data_table_drop(disp_data_table *table){disp_dealloc(table->name);for(size_t i=0;i<table->field_count;i++){disp_string_drop(&table->field_names[i]);disp_string_drop(&table->field_types[i]);}disp_dealloc(table->field_names);disp_dealloc(table->field_types);disp_dealloc(table->required);disp_dealloc(table->primary);for(size_t i=0;i<table->rows_len;i++)disp_json_drop(&table->rows[i]);disp_dealloc(table->rows);*table=(disp_data_table){0};}
...
static bool disp_data_decode(disp_database_state *state,const unsigned char *bytes,size_t length,disp_native_string *error){
...
static bool disp_data_ensure_schema(disp_database_state *state,const char *schema,size_t schema_len,const char *create_sql,size_t create_len,const char *inspect_sql,size_t inspect_len,const char *const *names,const char *const *types,const bool *required,const bool *primary,size_t count,disp_native_string *error){
if(state&&state->native)return disp_data_native_schema(state,schema,schema_len,names,types,required,primary,count,error);
#ifdef DISP_DATABASE
uint64_t ignored=0;
