...
typedef struct {disp_native_json key;size_t row;} disp_data_index_entry;
typedef struct {char *name;size_t name_len;disp_native_string *field_names;disp_native_string *field_types;bool *required;bool *primary;bool *unique;bool *indexed;disp_data_index_entry **indexes;size_t *index_lens;size_t field_count;size_t primary_index;disp_native_json *rows;size_t rows_len;size_t rows_cap;} disp_data_table;
...
#endif
static void disp_data_table_drop(disp_data_table *table){disp_dealloc(table->name);for(size_t i=0;i<table->field_count;i++){disp_string_drop(&table->field_names[i]);disp_string_drop(&table->field_types[i]);if(table->indexes&&table->index_lens){for(size_t j=0;j<table->index_lens[i];j++)disp_json_drop(&table->indexes[i][j].key);disp_dealloc(table->indexes[i]);}}disp_dealloc(table->field_names);disp_dealloc(table->field_types);disp_dealloc(table->required);disp_dealloc(table->primary);disp_dealloc(table->unique);disp_dealloc(table->indexed);disp_dealloc(table->indexes);disp_dealloc(table->index_lens);for(size_t i=0;i<table->rows_len;i++)disp_json_drop(&table->rows[i]);disp_dealloc(table->rows);*table=(disp_data_table){0};}
...
static bool disp_data_decode(disp_database_state *state,const unsigned char *bytes,size_t length,uint8_t allowed_field_flags,disp_native_string *error){
...
static void disp_data_indexes_clear(disp_data_table *table){if(!table->indexes||!table->index_lens)return;for(size_t field=0;field<table->field_count;field++){for(size_t index=0;index<table->index_lens[field];index++)disp_json_drop(&table->indexes[field][index].key);disp_dealloc(table->indexes[field]);table->indexes[field]=NULL;table->index_lens[field]=0;}}
static bool disp_data_indexes_rebuild(disp_data_table *table,disp_native_string *error){disp_data_indexes_clear(table);if(!table->indexes)table->indexes=(disp_data_index_entry**)disp_alloc_zeroed(table->field_count,sizeof(disp_data_index_entry*),_Alignof(disp_data_index_entry*));if(!table->index_lens)table->index_lens=(size_t*)disp_alloc_zeroed(table->field_count,sizeof(size_t),_Alignof(size_t));for(size_t field=0;field<table->field_count;field++){if(!table->primary[field]&&!table->unique[field]&&!table->indexed[field])continue;if(table->rows_len)table->indexes[field]=(disp_data_index_entry*)disp_alloc_zeroed(table->rows_len,sizeof(disp_data_index_entry),_Alignof(disp_data_index_entry));for(size_t row=0;row<table->rows_len;row++){disp_native_json key={0};disp_native_string *name=&table->field_names[field];if(!disp_json_get(&table->rows[row],name->data,name->len,&key)){disp_data_indexes_clear(table);*error=disp_owned_bytes("stored row is missing an indexed field",strlen("stored row is missing an indexed field"));return false;}size_t at=table->index_lens[field];while(at&&disp_data_index_compare(&table->indexes[field][at-1].key,&key)>0){table->indexes[field][at]=table->indexes[field][at-1];at--;}if((table->primary[field]||table->unique[field])&&((at&&disp_data_index_compare(&table->indexes[field][at-1].key,&key)==0)||(at<table->index_lens[field]&&disp_data_index_compare(&table->indexes[field][at].key,&key)==0))){disp_json_drop(&key);disp_data_indexes_clear(table);*error=disp_owned_bytes("stored table contains a duplicate indexed value",strlen("stored table contains a duplicate indexed value"));return false;}table->indexes[field][at]=(disp_data_index_entry){.key=key,.row=row};table->index_lens[field]++;}}return true;}
...
static bool disp_data_ensure_schema(disp_database_state *state,const char *schema,size_t schema_len,const char *create_sql,size_t create_len,const char *inspect_sql,size_t inspect_len,const char *const *names,const char *const *types,const bool *required,const bool *primary,const bool *unique,const bool *indexed,size_t count,disp_native_string *error){
if(state&&state->native)return disp_data_native_schema(state,schema,schema_len,names,types,required,primary,unique,indexed,count,error);
#ifdef DISP_DATABASE
uint64_t ignored=0;
