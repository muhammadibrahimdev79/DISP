...
static bool disp_tcp_stream_read(disp_native_tcp_stream *stream,size_t limit,disp_native_string *bytes,disp_native_string *error,int line,int column){if(!stream->state||atomic_load_explicit(&stream->state->closed,memory_order_acquire)){*error=disp_owned_bytes("TCP stream is closed",strlen("TCP stream is closed"));return false;}if(atomic_load_explicit(&stream->state->read_shutdown,memory_order_acquire)){*error=disp_owned_bytes("TCP read side is shut down",strlen("TCP read side is shut down"));return false;}if(!disp_tcp_claim(&stream->state->read_busy)){*error=disp_owned_bytes("TCP read is already in progress",strlen("TCP read is already in progress"));return false;}if(limit>16777216ULL)dv_panic("TCP read limit exceeds the 16 MiB safety limit",line,column);if(!limit){atomic_store_explicit(&stream->state->read_busy,false,memory_order_release);return true;}char *data=(char*)disp_alloc(limit,1);for(;;){
#ifdef _WIN32
...
static void disp_udp_socket_close(disp_native_udp_socket *socket){if(!socket||!socket->state)return;if(!atomic_exchange_explicit(&socket->state->closed,true,memory_order_acq_rel))disp_socket_close(socket->state->socket);}
static void disp_udp_socket_release(disp_udp_socket_state *state){if(atomic_fetch_sub_explicit(&state->refs,1,memory_order_acq_rel)!=1)return;atomic_thread_fence(memory_order_acquire);if(!atomic_load_explicit(&state->closed,memory_order_acquire))disp_socket_close(state->socket);disp_dealloc(state);}
...
static bool disp_udp_socket_receive(disp_native_udp_socket *socket,size_t limit,disp_native_udp_datagram *datagram,disp_native_string *error,int line,int column){if(limit>65535)dv_panic("UDP receive limit exceeds 65535 bytes",line,column);if(!socket->state||atomic_load_explicit(&socket->state->closed,memory_order_acquire)){*error=disp_owned_bytes("UDP socket is closed",strlen("UDP socket is closed"));return false;}if(!disp_tcp_claim(&socket->state->receive_busy)){*error=disp_owned_bytes("UDP receive is already in progress",strlen("UDP receive is already in progress"));return false;}size_t capacity=limit+1;uint8_t *data=(uint8_t*)disp_alloc(capacity,1);for(;;){struct sockaddr_storage source={0};socklen_t source_length=sizeof(source);
#ifdef _WIN32
...
static bool disp_udp_socket_send(disp_native_udp_socket *socket,const char *bytes,size_t length,const disp_native_socket_address *address,size_t *sent,disp_native_string *error){if(length>65507){*error=disp_owned_bytes("UDP datagram exceeds the 65507-byte payload limit",strlen("UDP datagram exceeds the 65507-byte payload limit"));return false;}if(!socket->state||atomic_load_explicit(&socket->state->closed,memory_order_acquire)){*error=disp_owned_bytes("UDP socket is closed",strlen("UDP socket is closed"));return false;}if(!disp_tcp_claim(&socket->state->send_busy)){*error=disp_owned_bytes("UDP send is already in progress",strlen("UDP send is already in progress"));return false;}struct sockaddr_storage destination={0};socklen_t destination_length=0;if(!disp_udp_resolve(address,false,socket->state->family,&destination,&destination_length,error)){atomic_store_explicit(&socket->state->send_busy,false,memory_order_release);return false;}for(;;){
#ifdef _WIN32
