...
typedef struct {char *name;size_t name_len;disp_native_string *field_names;disp_native_string *field_types;bool *required;bool *primary;bool *unique;disp_data_index_entry **indexes;size_t *index_lens;size_t field_count;size_t primary_index;disp_native_json *rows;size_t rows_len;size_t rows_cap;} disp_data_table;
...
#endif
static void disp_data_table_drop(disp_data_table *table){disp_dealloc(table->name);for(size_t i=0;i<table->field_count;i++){disp_string_drop(&table->field_names[i]);disp_string_drop(&table->field_types[i]);if(table->indexes&&table->index_lens){for(size_t j=0;j<table->index_lens[i];j++)disp_json_drop(&table->indexes[i][j].key);disp_dealloc(table->indexes[i]);}}disp_dealloc(table->field_names);disp_dealloc(table->field_types);disp_dealloc(table->required);disp_dealloc(table->primary);disp_dealloc(table->unique);disp_dealloc(table->indexes);disp_dealloc(table->index_lens);for(size_t i=0;i<table->rows_len;i++)disp_json_drop(&table->rows[i]);disp_dealloc(table->rows);*table=(disp_data_table){0};}
...
static bool disp_data_indexes_rebuild(disp_data_table *table,disp_native_string *error){disp_data_indexes_clear(table);if(!table->indexes)table->indexes=(disp_data_index_entry**)disp_alloc_zeroed(table->field_count,sizeof(disp_data_index_entry*),_Alignof(disp_data_index_entry*));if(!table->index_lens)table->index_lens=(size_t*)disp_alloc_zeroed(table->field_count,sizeof(size_t),_Alignof(size_t));for(size_t field=0;field<table->field_count;field++){if(!table->primary[field]&&!table->unique[field])continue;if(table->rows_len)table->indexes[field]=(disp_data_index_entry*)disp_alloc_zeroed(table->rows_len,sizeof(disp_data_index_entry),_Alignof(disp_data_index_entry));for(size_t row=0;row<table->rows_len;row++){disp_native_json key={0};disp_native_string *name=&table->field_names[field];if(!disp_json_get(&table->rows[row],name->data,name->len,&key)){disp_data_indexes_clear(table);*error=disp_owned_bytes("stored row is missing an indexed field",strlen("stored row is missing an indexed field"));return false;}size_t at=table->index_lens[field];while(at&&disp_data_index_compare(&table->indexes[field][at-1].key,&key)>0){table->indexes[field][at]=table->indexes[field][at-1];at--;}if((at&&disp_data_index_compare(&table->indexes[field][at-1].key,&key)==0)||(at<table->index_lens[field]&&disp_data_index_compare(&table->indexes[field][at].key,&key)==0)){disp_json_drop(&key);disp_data_indexes_clear(table);*error=disp_owned_bytes("stored table contains a duplicate indexed value",strlen("stored table contains a duplicate indexed value"));return false;}table->indexes[field][at]=(disp_data_index_entry){.key=key,.row=row};table->index_lens[field]++;}}return true;}
...
static bool disp_data_native_lookup(disp_database_state *state,const char *schema,size_t schema_len,size_t field,const disp_native_json *key,disp_native_json **rows,size_t *rows_len,size_t *rows_cap,disp_native_string *error){*rows=NULL;*rows_len=*rows_cap=0;disp_data_table *table=disp_data_native_table(state,schema,schema_len);if(!table){const char *message="DISP Data schema is not registered";*error=disp_owned_bytes(message,strlen(message));return false;}if(field>=table->field_count||(!table->primary[field]&&!table->unique[field])){const char *message="DISP Data plan selected an invalid index";*error=disp_owned_bytes(message,strlen(message));return false;}size_t row=SIZE_MAX;if(!disp_data_index_find(table,field,key,&row))return true;if(row>=table->rows_len){const char *message="DISP Data index contains an invalid row";*error=disp_owned_bytes(message,strlen(message));return false;}*rows=(disp_native_json*)disp_alloc_zeroed(1,sizeof(disp_native_json),_Alignof(disp_native_json));(*rows)[0]=disp_json_literal(table->rows[row].data,table->rows[row].len);*rows_len=*rows_cap=1;return true;}
