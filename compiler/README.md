...
- owned SQLite `Database` connections with prepared parameter binding, bounded JSON rows, explicit transactions, typed `DataError` failures, and rollback/close during deterministic cleanup
- nominal `data` schemas plus compiler-owned add/save/find/remove syntax lowered through typed HIR/MIR Data plans; `data memory` executes them directly on a DISP-owned native row store with bound-once external values, safe primary keys, limits, deterministic cleanup, and no SQL surface
